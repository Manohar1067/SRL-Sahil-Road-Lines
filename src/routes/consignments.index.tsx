...
import { Search, FilePlus2, ChevronLeft, ChevronRight, ArrowUpDown, X } from "lucide-react";
import {
...
function ConsignmentList() {
  ...
  const [sortKey, setSortKey] = useState<"date" | "netFreight" | "receiptNumber">("date");
  ...
  const filtered = useMemo(() => {
    ...
    return rows;
  }, [dispatches, q, memo, status, truck, driver, consignor, consignee, destination, material, dispatchDate, deliveryDate, docDate, invDate, sortKey, sortDir]);

  const totalPages = Math.max(1, Math.ceil(filtered.length / pageSize));
  const pageRows = filtered.slice((page - 1) * pageSize, page * pageSize);
  const toggleSort = (k: typeof sortKey) => {
    if (sortKey === k) setSortDir(sortDir === "asc" ? "desc" : "asc"); else { setSortKey(k); setSortDir("desc"); }
  };
  const resetFilters = () => {
    setQ(""); setMemo("");
  ...
  const SelectFilter = ({ value, onChange, label, options }: { value: string; onChange: (v: string) => void; label: string; options: string[] }) => (
        ...
        <CardContent className="space-y-4 p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              ...
              <Input value={q} onChange={(e) => { setQ(e.target.value); setPage(1); }} placeholder="Search across every field…" className="pl-9" />
            </div>
            <Input value={memo} onChange={(e) => { setMemo(e.target.value); setPage(1); }} placeholder="Dispatch Memo #" className="sm:max-w-[200px] font-mono" />
          ...
          <div className="overflow-x-auto rounded-lg border">
            <table className="min-w-full text-sm">
              <thead className="bg-muted/60 text-left text-xs uppercase tracking-wide text-muted-foreground">
                <tr>
                  <th className="cursor-pointer px-4 py-3" onClick={() => toggleSort("receiptNumber")}>Dispatch Memo # <ArrowUpDown className="inline h-3 w-3" /></th>
                  <th className="cursor-pointer px-4 py-3" onClick={() => toggleSort("date")}>Dispatch Date <ArrowUpDown className="inline h-3 w-3" /></th>
                  ...
                  <th className="cursor-pointer px-4 py-3 text-right" onClick={() => toggleSort("netFreight")}>Total <ArrowUpDown className="inline h-3 w-3" /></th>
